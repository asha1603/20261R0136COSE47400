...
def ssim(recon: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Global grayscale SSIM for [0, 1] tensors, kept compatible with current repo."""
    c1, c2 = 0.01 ** 2, 0.03 ** 2
    dims = (1, 2, 3)
    ...
    sigma_xy = (
        (recon - mu_x[:, None, None, None]) * (target - mu_y[:, None, None, None])
    ).mean(dim=dims)
...
def format_confusion_matrix(matrix: torch.Tensor) -> str:
    return "\n".join("[" + ", ".join(str(int(v)) for v in row.tolist()) + "]" for row in matrix)
